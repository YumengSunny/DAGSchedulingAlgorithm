...
from eopa import EOPA_Algorithm
...
def trace(msglevel, timestamp, message):
    if msglevel == 0:
        logging.debug("t = " + str(timestamp) + ": " + message)
    elif msglevel == 1:
        logging.info("t = " + str(timestamp) + ": " + message)
    elif msglevel == 2:
        logging.warning("t = " + str(timestamp) + ": " + message)
    elif msglevel == 3:
        logging.error("t = " + str(timestamp) + ": " + message)
    else:
        pass
...
def EO_v1():
    ...
    _, EO_V_C = EOPAObject.find_longest_path_dfs(EO_G, EO_V[0], EO_V[-1], wcet)
    ...
    EO_V_A = graph.find_associative_nodes(EO_G, candidate, critical_nodes)
...
def sched(dag, number_of_cores, algorithm="random", execution_model="WCET", T_MAX=1000000000):
    ...
    if algorithm == "eligibility":
        Prio = EOPAObject.Eligiblity_Ordering_PA(dag.G, dag.C_dict)
        ...
        for m in range(number_of_cores):
            if cores[m].idle:
                ...
                if r_queue:
                    ...
                    cores[m].assign(tau)
                    workload_queue.append(tau.C)

                    r_queue.remove(task_idx)
                    trace(0, t, "Job {:d} assgined to Core {:d}".format(task_idx, m))
                    pair_tuple = (task_idx, t)
                    sched_dict[m].append(pair_tuple)
    ...
    return makespan, sched_dict, workload_queue
...
if __name__ == "__main__":
    ...
    C = {1: 3, 2: 79, 3: 74, 4: 73, 5: 76, 6: 71, 7: 99, 8: 5}
    for m in [2, 3, 4, 5, 6, 7, 8]:
        R_all = []

        # for idx in range(1000):

        G_dict, C_dict, C_array, lamda, VN_array, L, W = EOPAObject.load_task(G, C)
        dag = DAGTask(G_dict, C_array)

        # find the high watermark of random
        # R1是最终总的运行时间
        # sched_dict是记录每一个核上运行的节点及其开始时间的dict {核编号: [(节点编号, 此节点开始时间)]}
        # workload_queue是每一个节点对应的运行时间
        R1, sched_dict, workload_queue = sched(dag, number_of_cores=m, algorithm="eligibility", execution_model="WCET")

        print("{}, {}, {}".format(R1, sched_dict, workload_queue))
